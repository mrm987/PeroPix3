...
def strip(image_bytes: bytes, fmt: str = "PNG", quality: int = 95) -> bytes:
    ...
    if arr.ndim == 3 and arr.shape[2] == 4:
        # ★★**묻지 않고 언제나 민다** (사용자 지적 2026-08-31: *"체크했으면 확인할 것 없이
        #   그냥 무조건 파괴하면 되는 것 아니냐"*). 맞다 — 알파의 **맨 끝 한 비트**만 건드리므로
        #   숨은 데이터가 있든 없든 잃는 것이 없고, 우리가 못 알아보는 방식으로 심긴 것도 함께
        #   사라진다. 예전에는 「불투명한가」·「우리가 아는 시그니처인가」를 따졌는데, 그 판정이
        #   틀리면 **조용히 안 지워졌다**: 한두 픽셀만 254 아래여도 통째로 건너뛰었고
        #   (실측 200장 중 34장), 투명 배경 그림은 배경의 알파가 이미 0·1 이라 밀어도 그대로였다.
        # ★양 끝은 **정확히 지킨다** — 완전 불투명(254·255)은 255 로, 완전 투명(0·1)은 0 으로.
        #   그래서 투명 배경 그림의 배경은 투명한 채로 남고, 불투명한 그림은 불투명한 채로 남는다.
        #   가운데 값(반투명 가장자리)은 끝 비트를 0 으로 내린다. 어느 쪽이든 최대 1/255 다.
        a = arr[:, :, 3]
        arr[:, :, 3] = np.where(a >= 254, 255, np.where(a <= 1, 0, a & 0xFE))
