# -*- coding: utf-8 -*-
"""태그 색인 — 그림마다 **긍정 프롬프트 원문**을 곁파일에 모아 둔다 (사용자 지시 2026-08-31:
작가 태그를 섞어 쓰다 보면 어느 작가가 어느 그림체였는지 잊는다).

★뿌리를 인자로 받는다. 지금 쓰는 자리는 **보관함**(`KEEP_DIR`) 하나이고, 창구는
  `/api/keep/tags/*` 다. 아웃풋 루트를 훑던 쪽은 그것을 쓰던 태그 서랍과 함께 걷혔다
  (사용자 지시 2026-09-21).
